...
```
Going to treat 'banka' as a train dataset , banka_full as a test dataset. 'banka' will be used to train the model, and the model will then be tested on banka_full, to guage how accurate it is.


Create first model using binomial glm (general linear model) :
```{r}
...
Also less likely to subscribe Jan-Mar vs any other month. More likely to subscribe if previously contacted. Blue collar are the mosth likely job to say yes to subscribe.
...
#86.09% correct when predictions for bank-additional-full are compared to actual
...
#try to better model by switching what variables are used in model. Only use top significant values.
glm.new <- glm( output ~ 
                  #age + 
                  #job +
                  #marital +
...
Model "glm.new" has an acceptable result of 86.29% accuracy. This number, shows that the model fits the dataset, but is not to close to overfitting.


Plot relationship between clients subscribing a term deposit(y) and the metrics used in the model:
```{r}
