...
glm.new <- glm( y ~ age + 
                  ...
                  cons.conf.idx +
                  nr.employed
                , data = banka_full , family = binomial) #Note: Used binomial because output is binary
...
```{r}
#predict if the client subsribed to a term deposit
glm.probs <- predict(glm.new, banka, type = "response")
SubscribedPred =rep( 'no' , nrow(banka))
SubscribedPred[ glm.probs > .5] = 'yes'
results <- data.frame( banka$ID, SubscribedPred, banka$y)
results <- results %>% mutate( match = ifelse( SubscribedPred == banka.y , 1 , 0))
head(results)

#how well does glm.probs fit test
table(results$Subscribed, banka$y)
sum(results$match)/nrow(results)
# 30% correct when predictions for bank-additional-full are compared to actual
```
